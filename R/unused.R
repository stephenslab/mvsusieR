...
# columns are removed for duplicates with what other columns.
rm_collinear = function (mat, ...) {
  ...
  if (any(bool_coll)) {
    ...
    # using fromLast, the previously NOT duplicated columns (FALSE)
    ...
    assoc_coll = which(bool_with_coll)[match(data.frame(mat_coll),
