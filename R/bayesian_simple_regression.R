...
BayesianSimpleRegression <- R6Class("BayesianSimpleRegression",
  public = list(
    ...
    fit = function(d, prior_weights = NULL, use_residual = FALSE, save_summary_stats = FALSE, save_var = FALSE, estimate_prior_variance_method = NULL, check_null_threshold=0, precomputed_quantity=NULL) {
      ...
      # posterior
      post_var = (1/private$prior_variance_scalar + 1/sbhat2)^(-1) # posterior variance
      ...
      # Bayes factor
      private$.lbf = dnorm(bhat,0,sqrt(private$prior_variance_scalar+sbhat2),log=TRUE) - dnorm(bhat,0,sqrt(sbhat2),log=TRUE)
  ...
  active = list(
    ...
    prior_variance = function(v) {
      if (missing(v)) private$prior_variance_scalar
      else private$prior_variance_scalar = v
    },
  ...
  private = list(
    ...
    estimate_prior_variance_optim = function(betahat, shat2, prior_weights, ...) {
      ...
      if(private$neg_loglik_logscale(log(private$prior_variance_scalar),betahat,shat2,prior_weights) < 
