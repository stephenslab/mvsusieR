  ...
  public = list(
    initialize = function (Z, R = NULL, eigenR=NULL, tol) {
      ...
      if (is.null(R) & is.null(eigenR))
        stop("At least one of R and eigen-decomposition of R should be ",
             "provided")
      if (!is.null(eigenR)) {
        if (any(names(eigenR) != c("values","vectors")))
          stop("eigen-decomposition of R must contains 2 elements with the ",
               "following names: values, vectors")
        if (nrow(eigenR$vectors) != nrow(Z))
          stop("The dimension of eigenvectors of R does not agree with ",
               "expected")
        .eigenR <<- eigenR
      }
      if (!is.null(R)) {
