...
Note that when only *z*-scores and the LD matrix are provided, the
effect estimates from `coef()` are on the *standardized* scale.
To obtain coefficients on the original scale that exactly match
`mvsusie()`, provide effect estimates (`Bhat`), standard errors
(`Shat`), and the sample variance of Y (`varY`). See the
[input data formats](input_data_format.html) vignette for details.
