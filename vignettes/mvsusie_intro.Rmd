...
fine-mapping data set with 500 samples, 1000 variants and 20 traits.
...
tolerance (a number closer to zero. Our default value is `1e-3`).
...
## Which traits are affected?

The "effect plot" summarizes which traits are affected by each
causal SNP. Point size indicates effect magnitude and color
indicates sign. Only effects with small local false sign rate
(LFSR) are shown.
...
For more details on interpreting mvSuSiE output---including
