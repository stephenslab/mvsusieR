...
This vignette demonstrates `mmbr` in the context of prediction. We first simulate three phenotypes
($y_1$, $y_2$, $y_3$) and genotypes at $P = 1000$ loci ($X$) for $N = 500$ individuals. Then, the goal is
to predict phenotypes from genotypes for $N_{test} = 100$ individuals, training our model on the remaining 
$N_{training} = 400$ individuals.
...
and compute the prior canonical covariance matrices.

```{r}
L <- 10
test_num <- 1:100
prior_var <- mmbr:::create_cov_canonical(3)
prior_covar <- mmbr:::MashInitializer$new(prior_var, 0.2)
```

Then, we fit the `mmbr` model to the training set.

```{r}
y <- cbind(y1[-test_num], y2[-test_num], y3[-test_num])
m <- mmbr::msusie(X[-test_num, ], y, L=L, prior_variance=prior_covar)
```
...
```{r}
pred <- predict.mmbr(m, X[test_num, ])
