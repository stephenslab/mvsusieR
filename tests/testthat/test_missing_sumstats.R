...
test_that("With full observation, the estimated prior variance are same for DenseDataYMissing and DenseData", with(simulate_multivariate(r=3, center_scale = F), {
  ...
  expect_equal(fit1$b2, fit3$b2, tolerance = 1E-8)
  expect_equal(fit1$coef, fit3$coef, tolerance = 1E-8, check.attributes = FALSE)
  expect_equal(fit1$V, fit3$V, tolerance = 1E-8)
}))
