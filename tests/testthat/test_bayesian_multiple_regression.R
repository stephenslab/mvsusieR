...
test_that("mmbr is identical to susieR", with(simulate_univariate(), {
    ...
    B = BayesianMultipleRegression$new(ncol(X), 1, V, estimate_prior_variance = FALSE)
    B$fit(d, prior_weights = NULL)
    ...
    B = BayesianMultipleRegression$new(ncol(X), 1, V, estimate_prior_variance = TRUE)
    B$fit(d, prior_weights = NULL)
